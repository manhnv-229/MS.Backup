...
const commonJs = {
    ...
    convertDateTimeToStringDateTime: function (value) {
        try {
            if (!value) return "";
            value = new Date(value);
            var date = value.getDate();
    ...
    showErrorMessenger() {
        ...
        if (payload.length == 0) {
            ...
            for (let index = 1; index < payload.length; index++) {
                ...
                if (item && (typeof item === 'string' || item instanceof String || typeof item == 'number')) {
                    msg.push(item);
        ...
        store.dispatch(SET_ERROR_MSG, { title: title, msg: msg, type: MISAEnum.MsgType.Error, confirm: callback });
    },
    /**
    ...
    showMessenger({ title, msg, type, confirm, showCancelButton }) {
        store.dispatch(SET_ERROR_MSG, { title: title || "Thông báo", msg: msg || "Có lỗi xảy ra", type: type || MISAEnum.MsgType.Info, confirm: confirm, showCancelButton: showCancelButton });
    },
