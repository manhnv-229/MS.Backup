      ...
      <m-table-layout>
        ...
        <template v-slot:toolbar>
          <div class="toolbar__left">
            <m-input-search width="300px" placeholder="Tìm kiếm" v-model="keySearch"></m-input-search>
          </div>
          <div class="toolbar__right">
            <button class="btn-icon icon icon-24 icon--mail-statistic" title="GỬI THỐNG KÊ HÔM NAY" @click="sentStatistic()">
            </button>
            <button class="btn-icon icon icon-20 icon--backup" title="SAO LƯU DỮ LIỆU" @click="backupDatabase()">
            </button>
            <button class="btn btn-refresh" title="LẤY DỮ LIỆU MỚI" @click="loadData()">
        ...
        <!-- BẢNG DỮ LIỆU -->
        <template v-slot:container>
          <m-table ref="tbOrganizations" :data="organizations" empty-text="Không có dữ liệu" width="100%" height="100%">
            ...
            <m-column prop="OrganizationName" fixed="left" label="Tên cửa hàng">
              <template #default="scope">
                <div class="table-cell org-name">
                  <span class="org-name__icon org--confirm" v-if="scope.row.IsConfirm"><i
            ...
            <m-column prop="Address" label="Địa chỉ" v-if="!isMobile"></m-column>
            <m-column width="50" fixed="right">
              <template #default="scope">
                <div class="button-column">
                  <button class="btn--table-mini --color-edit" :title="scope.row.OrganizationName"
                    @click="onUpdate(scope.row)">
                    <i class="icofont-ui-edit"></i>
                  </button>
                  <button v-if="!scope.row.IsConfirm" class="btn--table-mini --color-red"
                    :title="scope.row.OrganizationName" @click="onDelete(scope.row)">
        ...
        <template v-slot:footer>
          <m-paging entityName="cửa hàng" :totalRecords="totalRecords" :pageSizeData="[10, 20, 50, 100, 200]"
            v-model:limit="limit" v-model:offset="offset" @onChangePagesize="loadData" @onPrevPage="loadData"
            @onNextPage="loadData" @onInitPaging="onInitPaging"></m-paging>
        </template>
      </m-table-layout>
    </template>
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      this.maxios
        .get(
          `organizations/paging?limit=${this.limit}&offset=${this.offset}&key=${key || ""
          }`
        )
        .then((res) => {
    ...
    backupDatabase() {
      this.commonJs.showConfirm(
        ...
        () => {
          this.maxios
            .post(`organizations/backup`);
        }
      );
    },
    sentStatistic(){
      this.commonJs.showConfirm(
        `Việc này sẽ thực hiện gửi báo cáo doanh thu tự động trong <b>ngày hôm nay (${commonJs.formatDate(new Date())})</b> cho toàn bộ các đơn vị. <br>
        ...
        () => {
          ...
          this.maxios
            .get(`organizations/statistics?date=${year}-${monthString}-${dateString}&sentEmail=true`);
        }
      );
    }
  },
