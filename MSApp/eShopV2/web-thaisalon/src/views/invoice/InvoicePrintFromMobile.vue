  ...
  <m-dialog @onClose="onClose">
    <template v-slot:content>
      <div v-show="!showPrintPreview" id="invoice" class="invoice">
        ...
        <div class="invoice__header">
          <div class="header__left">
            <div class="org-info">
              <div class="org__name">{{ invoice.Organization?.OrganizationName }}</div>
              <div class="org__address">
                <div class="info__icon"><i class="icofont-home"></i></div>
                <div class="info__text">{{ invoice.Organization?.Address }}</div>
              </div>
              <div class="org__mobile">
                <div class="info__icon"><i class="icofont-ui-cell-phone"></i></div>
                <div class="info__text">{{ invoice.Organization?.PhoneNumber }}</div>
              </div>
            </div>
          </div>
          <div class="header__right">
            <div class="org__description">{{ invoice.Organization?.Description }}</div>
          </div>
        </div>
        <!-- NAME -->
        <div class="invoice__name">
          ...
          <div class="customer">
            <div class="customer__name">Tên khách hàng: {{ invoice.CustomerName }}</div>
        ...
        <div class="invoice__products">
          ...
          >
            ...
            <m-column prop="ProductName" label="SP/dịch vụ" width="130px">
              <template #default="scope">
                <div
                  class="flex"
                  :class="{
                    'is-delete': scope.row.EntityState == Enum.EntityState.DELETE,
                  }"
                >
            ...
            <m-column label="Thành tiền" align="right" fixed="right" width="120px">
              <template #default="scope">
        ...
        <div class="invoice__money">
          <div class="money">
            ...
            <div class="money__text">{{ commonJs.formatMoney(invoice.TotalMoney) }}</div>
          </div>
        </div>
      ...
      <div v-if="showPrintPreview" id="print-element">
        <div class="p-org">
          <div class="p-org__name">{{ invoice.Organization?.OrganizationName }}</div>
          <div class="p-org__description">{{ invoice.Organization?.Description }}</div>
          <div class="p-org__address">{{ invoice.Organization?.Address }}</div>
          <div class="p-org__phone">
            <i class="icofont-ui-cell-phone"></i>: {{ invoice.Organization?.PhoneNumber }}
          </div>
        </div>
        ...
        <div class="p-invoice__date">
          <label>Ngày:</label> {{ commonJs.formatDateTime(invoice.InvoiceDate) }}
        </div>
        ...
        <div class="p-products">
          <div class="p-product" v-for="(p, index) in invoice.InvoiceDetail" :key="index">
        ...
        <div class="p-total-money">
          ...
          <div class="p-money__value">{{ commonJs.formatMoney(invoice.TotalMoney) }}</div>
        </div>
        <div class="p-person">
