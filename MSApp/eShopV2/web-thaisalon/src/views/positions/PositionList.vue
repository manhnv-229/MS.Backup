<template>
          ...
          >
            ...
            <m-column width="50" fixed="right">
              <template #default="scope">
                <div v-if="!scope.row.IsSystem || scope.row.IsSystem == 0" class="button-column">
                  <button
  ...
  <PositionDetail v-if="showDetail" @onClose="onCloseDetail" :id="idSelected"></PositionDetail>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      this.maxios
        .get(`positions/paging?limit=${this.limit}&offset=${this.offset}&key=${key || ""}`)
        .then((res) => {
