...
namespace MS.Infrastructure.Data
{
    ...
    {
        public MSLicenseRepository(MySqlDbContext sqlDbContext) : base(sqlDbContext)
        {
        }
        ...
        public async Task<MSLicense > GetLicenseByUserName(string userName)
        {
            ...
            var license = await DbContext.Connection.QueryFirstOrDefaultAsync<MSLicense >(sql, paremeters);
            return license;
        }
        ...
        public async Task<MSLicense> GetLicenseByOrganizationId(Guid orgId)
        {
            ...
            var license = await DbContext.Connection.QueryFirstOrDefaultAsync<MSLicense>(sql, paremeters, transaction:DbContext.Transaction);
            return license;
        }
        ...
        public async Task<int> DeleteLicenseByOrganizationId(Guid orgId)
        {
            ...
            var paremeters = new DynamicParameters();
            paremeters.Add("@OrganizationId", orgId);
            var res =  await DbContext.Connection.ExecuteAsync(sql,paremeters,transaction:DbContext.Transaction);
            return res;
        }
    }
}
