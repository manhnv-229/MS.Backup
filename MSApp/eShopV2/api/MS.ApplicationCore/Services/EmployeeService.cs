...
namespace MS.ApplicationCore.Services
{
    public class EmployeeService : BaseService<Employee>, IEmployeeService
    {
        ...
        public EmployeeService(IEmployeeRepository repository, IUnitOfWork unitOfWork, IMapper mapper, IJwtUtils tokenUtils, IFileTransfer fileTransfer, ICommonFunction commonFunc) : base(repository, unitOfWork, mapper)
        {
        ...
        public override async Task<int> RemoveAsync(object key)
        {
            ...
            {
                throw new MISAException(HttpStatusCode.Forbidden, "Bạn không được phép xóa chính mình.");
            }

            if (loginUserInfo.HighestRole > currentEmployeeInfo.HighestRole)
            {
                throw new MISAException(HttpStatusCode.Forbidden, "Bạn không được phép xóa thành viên này.");
            }
        ...
        public async Task<int> AddAsync(EmployeeInfo employeeRequest, IFormFile file)
        {
            ...
            employee.EntityState = MSEnums.EntityState.ADD;
            employeeRequest.EntityState = MSEnums.EntityState.ADD;
            ...
            if (Errors.Count == 0)
            {
                ...
                if (rowAddEmployee > 0 && rowAddUser)
                {
                    ...
                    if (!roleAdd)
                    {
                        await _fileTransfer.DeleteFile(employee.AvatarLink);
                        throw new MISAException(HttpStatusCode.InternalServerError, "Không thể cấp quyền cho thành viên. Vui lòng liên hệ Quản trị viên để được trợ giúp.");
                    }
                    UnitOfWork.Commit();
                ...
                {
                    await _fileTransfer.DeleteFile(employee.AvatarLink);
                    throw new MISAException(HttpStatusCode.InternalServerError, "Không thể thêm mới thành viên do lỗi hệ thống, vui lòng liên hệ QTV để được trợ giúp!");
                }
            }
            else
            {
                throw new MISAException(HttpStatusCode.BadRequest, Errors);
            }
        }
        ...
        public async Task<int> UpdateAsync(EmployeeInfo employeeRequest, IFormFile file)
        {
            var employee = Mapper.Map<EmployeeInfo, Employee>(employeeRequest);
            var user = Mapper.Map<EmployeeInfo, User>(employeeRequest);
            employee.EntityState = MSEnums.EntityState.UPDATE;
            employeeRequest.EntityState = MSEnums.EntityState.UPDATE;
        ...
        private async Task<bool> AddPermission(EmployeeInfo employeeRequest, User user)
        {
            ...
            if (roleId == null && employeeRequest.EntityState == MSEnums.EntityState.ADD)
            {
            ...
            if (employeeRequest.EntityState == MSEnums.EntityState.UPDATE)
            {
        ...
        private async Task ProcessAvatar(Employee employee, IFormFile file)
        {
            if (file != null && file.Length > 0)
            {
                ...
                else
                {
                    ...
                    employee.AvatarLink = await _fileTransfer.UploadFile(file, "avatars", employee.EmployeeId.ToString());
                }
            }
            if (file == null && String.IsNullOrEmpty(employee.AvatarLink))
            {
                ...
                employee.AvatarLink = await _fileTransfer.UploadFile(newFile, "avatars", employee.EmployeeId.ToString());
            }
        }
