...
namespace MS.Core.Services
{
    public class RefService : BaseService<Ref>, IRefService
    {
        ...
        protected async override Task ValidateObjectCustom(Ref entity)
        {
            var refType = entity.RefType;
            if (refType == RefType.IN_WAREHOUSE)
            {
                var stockInventories = entity.StockInventory;
            ...
            {
                var inventoryItems = entity.RefDetail;
                if (inventoryItems == null || inventoryItems.Count == 0)
                {
                    AddErrors("RefDetail", "Hóa đơn cần có ít nhất 1 hàng hóa.");
                }
            }

            if (entity.ChangeAmount < 0)
            {
                AddErrors("ChangeAmount", "Số tiền thanh toán chưa đủ.");
            }
        }
        ...
        protected async override Task AfterSave(Ref entity, bool saveSuccess = true)
        {
