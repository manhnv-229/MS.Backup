<template>
  <m-page title="Thông tin thuê bao" :rightSlot="true">
  ...
  <DeleteTenantProgress :item="lastTenantDelete" v-if="showDeleteProgress" @onFinish="onDeleteFinish"></DeleteTenantProgress>
</template>
<script>
import router from "@/router";
import debounce from "@/scripts/debounce";
import DeleteTenantProgress from "./DeleteTenantProgress.vue";
export default {
  name: "TenantList",
  components:{DeleteTenantProgress},
  created() {
    this.loadData();
  },
  watch: {
    ...
    async onDelete(tenant) {
      this.commonJs.showConfirm(
        ...
        () => {
          ...
          this.maxios.delete(`tenants/${tenant.TenantId}`, () => {
            this.loadData();
          },true);
        }
      );
