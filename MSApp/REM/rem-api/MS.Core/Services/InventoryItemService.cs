...
namespace MS.Core.Services
{
    public class InventoryItemService : BaseService<InventoryItem>, IInventoryItemService
    {
        ...
        IDrawTool _drawTool;
        public InventoryItemService(IInventoryItemRepository repository, IUnitOfWork unitOfWork, IMapper mapper, IFileTransfer fileTransfer, IConfiguration config, IHubContext<NotificationHub> notificationHub, IDrawTool drawTool) : base(repository, unitOfWork, mapper)
        {
                    ...
                    if (_config != null && configMaxSize != null)
                    {
                        ...
                        if (imgFile != null)
                        {
                            ...
                            if (imgFile.Length > imgMaxSize)
                            {
                                ...
                                //    }
                                //}
                                var imgResize = _drawTool.ResizeImg(imgFile);
                                inventoryItem.ImgPath = await _fileTransfer.UploadFileAsync(imgResize, "inventories", imgFileName);
                            }
                            else
                            {
        ...
        public override async Task<int> UpdateAsync(dynamic entity, object pks, IFormCollection? formData = null)
        {
            var entityObject = entity as InventoryItem;
            if (entityObject != null)
            {
                return await base.UpdateAsync(entityObject, pks, formData);
            }
            else
            {
                if (formData != null)
                ...
                {
                    AddErrors("InventoryItem", "Đơn hàng không được để trống.");
                    throw new MSException(HttpStatusCode.BadRequest, Errors);
                }
            }
