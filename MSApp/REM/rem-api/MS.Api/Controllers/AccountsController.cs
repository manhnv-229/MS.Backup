...
namespace Ms.Api.Controllers
{
    public class AccountsController : BaseController<User>
    {
        ...
        public AccountsController(IUserRepository repository, IUserService baseService, IJwtUtils jwtUntils, IConfiguration configuration, IUnitOfWork unitOfWork, IMapper mapper, IShareData shareData) : base(repository, baseService)
        {
        ...
        public async Task<IActionResult> GetClassInfo()
        {
            ...
            var orgId = HttpContext?.User?.Claims?.First(x => x.Type == "OrganizationId").Value;
            if (orgId != null)
            {
                var orgInfo = await _unitOfWork.Organizations.GetByIdAsync(orgId);
                return Ok(orgInfo);
            }
            else
            {
        ...
        public async Task<IActionResult> ConfirmEmail(UserTokenConfirm user)
        {
            ...
            if (res)
                ...
                throw new MSException(System.Net.HttpStatusCode.BadRequest, $"Mã xác nhận không hợp lệ. Vui lòng nhập chính xác mã xác nhận của bạn được gửi tới Email. <br/> Lưu ý. Mã xác nhận chỉ có hiệu lực trong 24h.");
        }
        ...
        public async Task<IActionResult> ConfirmMultiEmail(IEnumerable<User> users)
        {
            ...
            {
                throw new MSException(System.Net.HttpStatusCode.NoContent, "Không có bản ghi nào được cập nhật.");
            }
        }
        ...
        public async Task<IActionResult> ChangePassword(UserChangePassword userChangePassword)
        {
            ...
            {
                throw new MSException(System.Net.HttpStatusCode.InternalServerError, "Không thể thay đổi mật khẩu của bạn, vui lòng thử lại sau hoặc liên hệ Quản trị viên để được trợ giúp.");
            }
        }
