...
namespace MISA.FM.Infrastructure.Repositories
{
    public class MariaDbContext : IDisposable, IMSDatabaseContext
    {
        ...
        public IDbConnection AppConnection { get; }
        public IDbTransaction Transaction { get; set; }
        IWebHostEnvironment _env;
        IConfiguration _configuration;
        public MariaDbContext(IConfiguration configuration, IWebHostEnvironment env, IHttpContextAccessor httpContextAccessor, IMemoryCache msMemoryCache)
        {
            try
            {
                ...
                ConnectionString = GetTenantConnectionString();
                AppConnection = new MySqlConnection(ConnectionString);
                AppConnection.Open();
                Console.WriteLine($"Connection is Openned!!! - {DateTime.Now.ToLongTimeString()}");
            }
            catch (MySqlException ex)
            {
                Console.WriteLine($"Connection is Error!!! - {DateTime.Now.ToLongTimeString()}");
        ...
        /// <summary>
        /// Lấy chuỗi kết nối tuỳ theo thuê bao (dựa vào domain)
        /// </summary>
        /// <returns></returns>
        /// <exception cref="MSMySqlException"></exception>
        private string GetTenantConnectionString()
        {
            var clientHost = _httpContext?.Request.Headers["Origin"].FirstOrDefault()?.Split("//").Last().Replace("/",string.Empty);
            if (clientHost != null)
            {
                //var clientHost = _httpContext.Request.Headers["clientHost"].FirstOrDefault();
                Console.WriteLine(clientHost);
                var subDomain = clientHost?.Split('.')[0];
            }
            ...
            {
                return _configuration.GetConnectionString("MSDevConnection");
                //tenantConnectionString = _configuration.GetConnectionString("MSDevTenantConnection");
            }
            else if (_env.IsProduction())
            {
                tenantConnectionString = _configuration.GetConnectionString("MSProTenantConnection");
            }
            else
            {
                tenantConnectionString = _configuration.GetConnectionString("MSDevTenantConnection");
            ...
            {
                throw new MSMySqlException(System.Net.HttpStatusCode.InternalServerError, "Thông tin dữ liệu thuê bao không tồn tại, vui lòng liên hệ Mạnh Software để được trợ giúp.");
            }

            // Lấy dữ liệu thuê bao:
            // Kiểm tra xem có trong cache không, không có thì lấy từ db rồi lưu cho lần sau:
            var catalogs = _msMemoryCache.GetOrCreate(_configuration["MSCatalogDb_Cache"], entry =>
            {
                // Set cache entry options
                entry.SetAbsoluteExpiration(TimeSpan.FromDays(30));// HẾT HẠN SAU 30 NGÀY
                entry.SetSlidingExpiration(TimeSpan.FromDays(15));//TRONG 15 NGÀY KHÔNG TRUY CẬP THÌ HẾT HẠN, KỂ CẢ THỜI GIAN AbsoluteExpiration CÒN 
                entry.SetPriority(CacheItemPriority.NeverRemove);
                entry.Priority = CacheItemPriority.High;
                // Return product as cache entry value
                return GetTenantCatalogList(tenantConnectionString);
            });
            var tenantCatalog = catalogs?.Where(item => $"{item.SubDomain}.{item.RootDomain}" == clientHost).FirstOrDefault();
            if (tenantCatalog == null)
            {
                // Khi catalog bị null có thể do cache, nên để đảm bảo thì làm mới cache phát:
                var catalogListRefresh = GetTenantCatalogList(tenantConnectionString);
                _msMemoryCache.Set("MSCatalogDb", catalogListRefresh);
                tenantCatalog = catalogListRefresh?.Where(item => $"{item.SubDomain}.{item.RootDomain}" == clientHost).FirstOrDefault();
                // ---> Vẫn null nữa thì báo lỗi:
                if (tenantCatalog == null)
                {
                    throw new MSMySqlException(System.Net.HttpStatusCode.InternalServerError, "Thông tin dữ liệu thuê bao không tồn tại, vui lòng liên hệ Mạnh Software để được trợ giúp.");
                }
            }
            return $"Host={tenantCatalog.Server};" +
                     $"Port = 3306;" +
                     $"User Id={tenantCatalog.UserId};" +
                     $"Password={tenantCatalog.Password};" +
                     $"Database={tenantCatalog.DatabaseName};" +
                     $"charset=utf8;" +
                     $"convertzerodatetime=true;" +
                     $"Allow User Variables = True";
        }

        private IEnumerable<Catalog> GetTenantCatalogList(string connectionString)
        {
            Console.WriteLine("Get new data for db cache");
            // Query product from database
            var connection = new MySqlConnection(connectionString);
            connection.Open();
            var sql = "SELECT * FROM Catalog c";
            var catalogInDb = connection.Query<Catalog>(sql);
            connection.Close();
            return catalogInDb;
        }
        ...
        public void Dispose()
        {
            Console.WriteLine($"Connection is Closed!!! - {DateTime.Now.ToLongTimeString()}");
        ...
        public async Task<string> GetNewCodeDynamic<T>() where T : class
        {
            ...
            var newCode = await AppConnection.QueryFirstOrDefaultAsync<string>(storeName, parameters, transaction: Transaction, commandType: CommandType.StoredProcedure);
            return newCode;
        }
        ...
        public virtual async Task<IReadOnlyList<T>> GetAllAsync<T>() where T : class
        {
            ...
            var data = await AppConnection.QueryAsync<T>(sql, param: parameters, transaction: Transaction);
            return data.ToList();
        }
        ...
        {
            var data = AppConnection.Query<T>(sql, param: parameters, transaction: Transaction);
        ...
        {
            var data = AppConnection.QueryFirstOrDefault<T>(sql, param: parameters, transaction: Transaction);
            return data;
        }

        public async Task<T> QueryFirstOrDefaultAsync<T>(string sql, object parameters)
        {
            var data = await AppConnection.QueryFirstOrDefaultAsync<T>(sql, param: parameters, transaction: Transaction);
            return data;
        }
        ...
        public async Task<PagingData> GetFilterPaging<T>(string[] columnsSort, string[] columlsFilter, string keyFilter, int limit, int offset, string? tableName = null)
        {
            ...
            using (var multi = await AppConnection.QueryMultipleAsync(sql, parameters, transaction: Transaction))
            {
        ...
        public virtual async Task<int> AddAsync<T>(T entity, bool addChild = false) where T : BaseEntity
        {
            ...
            {
                newTransaction = true;
                Transaction = AppConnection.BeginTransaction();
            }
            // Thêm Master trước:
            var rowMasterAdd = await AppConnection.ExecuteAsync(sqlCommand, param: entity, transaction: Transaction);
        ...
        public virtual async Task<int> UpdateAsync<T>(T entity, bool updateChild = false) where T : BaseEntity
        {
            ...
            var rowUpdated = await AppConnection.ExecuteAsync(sqlCommand, param: entity, transaction: Transaction);
            if (updateChild == true)
            {
                var propKeyMasterName = GetPrimaryKeyName<T>();
                if (propKeyMasterName == null)
        ...
        public virtual async Task<int> DeleteAsync<T>(object id, bool deleteChil = false) where T : BaseEntity
        {
            ...
            var rowsDeleted = await AppConnection.ExecuteAsync(sqlCommand, param: parameters, transaction: Transaction);
            return rowsDeleted;
        }
        ...
        private IEnumerable<MSDbColumn> GetColumnsInDbOfTable(string tableName)
        {
            ...
            parameters.Add("@p_DbName", AppConnection.Database);
            var data = AppConnection.Query<MSDbColumn>(storeName, param: parameters, transaction: Transaction, commandType: CommandType.StoredProcedure);
            return data;
        }
        ...
        public async Task<string?> GetNewValueUnique<T>(string prefixCode, string? columnName = null) where T : class
        {
            ...
            var res = await AppConnection.ExecuteScalarAsync<string>(storeName, parameters, commandType: CommandType.StoredProcedure);
            return res;
        }
