...
namespace MS.Infrastructure.UnitOfWork
{
    public class MariaDbContextDEF : IDisposable, IMSDatabaseContext
    {
            ...
            if (rowMasterAdd > 0 && collections.Count > 0)
            {
                foreach (var collection in collections)
                {
                    ...
                    if (items != null)
                    {
                        ...
                        await AppConnection.ExecuteAsync(sql, items, transaction: Transaction);
                    }
                }
            }
            if (newTransaction)
        ...
        public virtual async Task<int> DeleteAsync<T>(T entity)
        {
            ...
            var sqlCommand = BuildDeleteQueryById<T>();
            var rowsDeleted = await AppConnection.ExecuteAsync(sqlCommand, param: entity, transaction: Transaction);
            return rowsDeleted;
        }
        #endregion
        ...
        public IEnumerable<DbColumn> GetColumnsInDbOfTable(string tableName)
        {
            ...
            var data = AppConnection.Query<DbColumn>(storeName, param: parameters, transaction: Transaction, commandType: CommandType.StoredProcedure);
            return data;
        }
        ...
        public Task<string?> GetNewValueUnique<T>(string prefixCode, string? columnName = null) where T : class
        {
            throw new NotImplementedException();
        }
        #endregion
