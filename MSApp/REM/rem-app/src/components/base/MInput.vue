<template>
  <div class="input-wrapper" :class="cls" :style="`width:${width}px;`" @click="onClick">
    <label v-if="label">{{ label }}
      <span v-if="required">(<span class="required">*</span>)</span>:</label>
    <div class="input-block">
      <span v-if="isShowClear" class="clear-button" :class="{ 'clear-button--left': align == 'right' }"
        @click="clearButtonOnClick"><i class="icofont-close"></i></span>
      <input :type="type" ref="minput" :required="required" :placeholder="placeholder" v-model="value" @input="onInput"
        @keydown="onKeyDown" @blur="onBlur" @focusin="onFocusIn" :maxlength="maxLength" :autocomplete="autocomplete"
        :disabled="disabled" :style="{ 'text-align': align }" class="input" :class="{
    ...
    <div v-if="inValid" class="validate-error">
      <div class="error__content">
        ...
        <div v-else>
          <span v-if="label"><b>{{ label }}</b></span><span v-else>này</span> không được phép để trống.
        </div>
      </div>
...
export default {
  ...
  props: {
    ...
    selectAllText: { type: Boolean, default: false, required: false },
  },
  emits: [
    "update:modelValue",
    "onValidate",
      ...
      this.$nextTick(function () {
        ...
        if (this.selectAllText) {
          ...
          if (value != null && value != undefined) {
            this.$refs.minput.setSelectionRange(0, value.length);
          }
        }
      });
    }
  ...
  methods: {
    ...
    onFocusIn() {
      this.$nextTick(() => {
        if (this.selectAllText && this.value) {
          this.$refs.minput.setSelectionRange(0, this.value.toString().length);
        }
      })
      this.$emit("onFocusIn", this.value);
    },
    clearButtonOnClick() {
