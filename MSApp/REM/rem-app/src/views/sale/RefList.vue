      ...
      <m-table-layout>
        ...
        <template v-slot:container>
          <m-table ref="tbProducts" :data="refs" empty-text="Không có dữ liệu" width="100%" height="100%">
            <m-column prop="RefNo" label="Mã hóa đơn" width="120px" class-name="m-cell-custom">
              <template #default="scope">
                <!-- <div>{{ scope.row.RefNo }}</div> -->
                <router-link class="link-underline" :to="{
                  name: 'RefDetailRouter',
                  params: { id: scope.row.RefId },
                }">{{ scope.row.RefNo }}</router-link>
                <div class="employee-name" v-if="scope.row.FullName">
                  ({{ scope.row.LastName }})
                </div>
              </template>
            </m-column>
            <m-column prop="RefDate" width="150px" label="Ngày lập" align="center"  class-name="m-cell-custom">
              <template #default="scope">
            ...
            <m-column prop="ItemList" min-width="200px" label="SP/DV" class-name="m-cell-custom">
              <template #default="scope">
                <div class="product-list">
                  <div class="product-list-item" v-for="(item, index) in processProductList(
                    ...
                    <span class="product-name">
                      <b>(<span v-if="item.ItemType === 'Inventory'">{{
                        item.Quantity
                      }}</span>
                        <span v-if="item.ItemType === 'Service'" class="is-service">{{
                          replaceDoubleQuotationMark(item.EmployeeName)
                        }}</span>)
                      </b>
                      <el-tooltip class="box-item" effect="customized" :content="`${replaceDoubleQuotationMark(item.ItemName)} (SL: ${item.Quantity
                        } x ${commonJs.formatMoney(item.UnitPrice)} = ${item.Total
                        })`" placement="top">
                        <span :class="{ 'is-service': item.ItemType === 'Service' }">{{
                          replaceDoubleQuotationMark(item.ItemName) }}</span>
                      </el-tooltip>
                      <!-- <b>({{ item.Quantity }})</b>{{ item.ItemName }}: -->
                    </span>
            ...
            <m-column prop="PaymentStatus" width="150px" label="Tình trạng" align="left">
              <template #default="scope">
                <div v-if="scope.row.PaymentStatus == 0" style="color: #ff0000">
                  Chưa thanh toán
                </div>
...
import { replaceDoubleQuotationMark } from "@/scripts/helper";
export default {
  name: "RefList",
  ...
  methods: {
    ...
    replaceDoubleQuotationMark(text) {
      return replaceDoubleQuotationMark(text);
    }
  },
  data() {
    return {
