<template>
  <m-page title="Danh mục chi nhánh" :rightSlot="true">
      ...
      <m-table-layout>
        ...
        <template v-slot:toolbar>
          ...
          <div class="toolbar__right">
            ...
            <button class="btn btn--default btn--refresh" @click="onAddNew">
              <i class="icofont-ui-add"></i> Thêm chi nhánh
            </button>
          </div>
        </template>
        <!-- BẢNG DỮ LIỆU -->
        <template v-slot:container>
          ...
          >
            ...
            <m-column prop="BranchCode" width="150px" label="Tên chi nhánh"></m-column>
            <m-column prop="BranchName" label="Tên chi nhánh"></m-column>
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      this.maxios
        .get(
          `branchs/paging?limit=${this.limit}&offset=${this.offset}&key=${
            key || ""
          }`
        )
        .then((res) => {
    ...
    onUpdate(branch) {
      this.idSelected = branch.BranchId;
      this.showDetail = true;
    },
    onDelete(branch) {
      this.commonJs.showConfirm(
        `Bạn có chắc chắn muốn xóa [${branch.BranchName}] không?`,
        () => {
          this.maxios.delete(`branchs/${branch.BranchId}`);
          this.loadData();
        }
      );
    },
