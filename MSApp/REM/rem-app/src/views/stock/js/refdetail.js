...
export default {
  ...
  created() {
    ...
    this.maxios.get(`/stocks`).then((res) => {
      this.stocks = res;
    });
    ...
    if (this.id) {
      this.maxios.get(`refs/${this.id}`).then((res) => {
        this.invoice = res;
        this.products = res.StockInventory;
      ...
      // Mặc định là phiếu bán hàng (phiếu nhập kho):
      this.invoice.RefType = Enum.RefType.IN_WAREHOUSE;
    ...
    totalMoneyComputed() {
      ...
      for (const product of this.products) {
        if (product.EntityState != Enum.EntityState.DELETE) {
          total += eval((product.CostPrice || 0) * (product.Quantity || 0));
        }
      }
      var obj = {
        value: total,
  ...
  methods: {
    onFocusInQuantity(record) {
      this.productEditting = record;
    },
    onFocusInCostPrice(record) {
      this.productEditting = record;
    },
    ...
    onDistributorAdd() {
      this.showDistributorAddForm = true;
    },
    onStockAdd(){
      this.showStockAddForm = true;
    },
    async onAddNewDistributorSuccess(distributor) {
      await this.$refs.cbxDistributor.reloadDataAndSetSelect(distributor.DistributorName);
    },
    async onAddNewStokcSuccess(stock){
      this.maxios.get(`/stocks`).then((res) => {
        this.stocks = res;
        this.$refs.cbxStock.reloadDataAndSetSelect(stock.StockName);
      });
    },
    ...
    onClickAllowDebitCheckbox(currentCheck) {
      if (!currentCheck && !this.invoice.AccountObjectId) {
        event.preventDefault();
        this.commonJs.showConfirm(
          "Vui lòng chọn đơn vị phân phối trước khi ghi công nợ."
        );
    ...
    onChangeDateItem(item){
      item.EntityState = Enum.EntityState.UPDATE;
    },
    ...
    // },
    onSave() {
      this.invoice.StockInventory = this.products;
    ...
    onSelectItemFilter(item) {
      // Đặt kho hàng:
      item.StockId = this.invoice.StockId;
      this.processItemSelected(item);
    },
    getSelectionInventoryItems(products) {
      // Chuyển tất cả các sản phẩm đã chọn thành đối tượng StockInventory:
    ...
    processItemSelected(product) {
      ...
      if (productExist) {
        ...
        productExist.TotalAmount =
          productExist.Quantity * productExist.CostPrice;
    ...
    onInputDistributorName(text) {
      this.invoice.AccountObjectName = text;
    },
    onSelectDistributor(value, text, distributor) {
      if (distributor) {
        this.invoice.AccountObjectPhoneNumber = distributor.PhoneNumber;
        this.invoice.ContactName = distributor.ContactName;
        this.invoice.ContactPhoneNumber = distributor.ContactPhoneNumber;
      }
    },
    onSelectStock(value, text, stock){
     
    },
    onCostPriceChanged(newCostPrice) {
      // Cập nhật trạng thái của Entity:
      this.productEditting.EntityState = Enum.EntityState.UPDATE;

      // Đầu tiên cập nhật tổng số tiền của sản phẩm:
      this.productEditting.TotalAmount =
        newCostPrice * this.productEditting.Quantity;
    ...
    onQuantityChanged(quantity) {
      ...
      // Đầu tiên cập nhật tổng số tiền của sản phẩm:
      this.productEditting.TotalAmount =
        quantity * this.productEditting.CostPrice;
      this.processAfterChangePriceOrQuantity();
    },
