  ...
  <m-dialog title="Nhóm hàng hoá" @onClose="onClose">
    <template v-slot:content>
      ...
      <div class="m-row">
        <m-text-area
          label="Mô tả"
          v-model="inventoryItemCategory.Description"
        ></m-text-area>
      </div>
      <div class="m-row">
        <div class="m-col">
          <m-checkbox
            ...
            v-model="inventoryItemCategory.IsSystem"
...
export default {
  ...
  methods: {
    onBlurName() {
      this.maxios
        .get(
          `/InventoryItemCategories/new-code?str=${
            this.inventoryItemCategory.InventoryItemCategoryName || ""
          }`
        )
        .then((newCode) => {
          this.inventoryItemCategory.InventoryItemCategoryCode = newCode;
        });
    },
    onClose() {
      this.$emit("onClose");
    ...
    onSave() {
      if (this.formMode == Enum.FormMode.ADD) {
        this.maxios
          .post(`inventoryItemCategories`, this.inventoryItemCategory)
          .then(() => {
            this.$emit("onClose");
        ...
        this.maxios
          .put(`inventoryItemCategories/${this.id}`, this.inventoryItemCategory)
          .then(() => {
            this.$emit("onClose");
          });
      }
