      ...
      <m-table-layout>
        ...
        <template v-slot:container>
          ...
          >
            ...
            <m-column
              prop="InventoryItemCategoryCode"
              width="150px"
              label="Mã nhóm"
            ></m-column>
            <m-column
...
export default {
  ...
  methods: {
    ...
    loadData() {
      ...
      this.maxios
        .get(
          `inventoryItemCategories/paging?limit=${this.limit}&offset=${
            this.offset
          }&key=${key || ""}`
        )
        .then((res) => {
    ...
    onDelete(inventoryItemCategory) {
      this.commonJs.showConfirm(
        ...
        () => {
          this.maxios
            .delete(
              `inventoryItemCategories/${inventoryItemCategory.InventoryItemCategoryId}`
            )
            .then((res) => {
              this.loadData();
            });
