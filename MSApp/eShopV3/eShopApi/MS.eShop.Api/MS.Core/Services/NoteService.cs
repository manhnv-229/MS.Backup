...
namespace MS.Core.Services
{
    public class NoteService : BaseService<Note>, INoteService
    {
        ...
        protected async override Task ValidateObjectCustom(Note entity)
        {
        }

        protected async override Task BeforeSave(Note entity)
        {
        ...
        protected async override Task AfterSave(Note entity)
        {
        }
        ...
        public async Task<Note> AddNewNote(Note note)
        {
            ...
            {
                throw new MISAException(System.Net.HttpStatusCode.InternalServerError, "Không thể thực hiện thêm mới ghi chú");
            }
        }
    }
}
