<template>
  <m-dialog title="Thông tin khách hàng" @onClose="onClose">
    <template v-slot:content>
      ...
      <div class="m-row">
        <div class="m-col">
          <m-combobox
            ...
            :showAddButton="true"
            @onClickExtButton="onCustomerGroupAdd"
            ref="cbxCustomerGroup"
          ></m-combobox>
        </div>
        <div class="m-col"></div>
      </div>
      ...
      <div class="m-row">
        ...
        <div class="m-col">
          <m-input label="Email" v-model="customer.Email" :isDisabled="false">
          </m-input>
        </div>
      </div>
      <div class="m-row">
  ...
  <CustomerGroupDetail
    v-if="showCustomerGroupFormAdd"
    @onClose="showCustomerGroupFormAdd = false"
    @onSaveSuccess="onAddGroupSuccess"
  ></CustomerGroupDetail>
</template>
<script>
import Enum from "@/scripts/enum";
import CustomerGroupDetail from "../customer-groups/CustomerGroupDetail.vue";
export default {
  name: "CustomerDetail",
  ...
  methods: {
    onCustomerGroupAdd() {
      this.showCustomerGroupFormAdd = true;
    },
    async onAddGroupSuccess(group) {
      await this.$refs.cbxCustomerGroup.reloadDataAndSetSelect(
    ...
    onSave() {
      if (this.formMode == Enum.FormMode.ADD) {
        this.maxios.post(`customers`, this.customer).then(() => {
          this.$emit("onClose");
          this.$emit("onSaveSuccess", this.customer);
        });
      } else {
  ...
  data() {
    return {
      customer: {},
      showCustomerGroupFormAdd: false,
    };
  },
};
</script>
